...
# 若 .env 裡還沒有 HEDGE_CONFIG_JSON，第一次啟動就用這組預設值，並自動寫入 .env
_DEFAULT_HEDGE_CONFIG = {
    '加密幣(Crypto)': {'symbols': ['MBT', 'BITU', 'BMNR', 'BMNU', 'CONL', 'ETHT', 'BIT', 'IBIT'], 'hedge_sym': 'MBT', 'upper_threshold': 2, 'lower_threshold': 0.5, 'micro_mult': 0.1, 'hedge_qty': 1, 'hedge_by_value': True},
    '玉米(Corn)': {'symbols': ['XC', 'OC', 'YC'], 'hedge_sym': 'XC', 'upper_threshold': -2.0, 'lower_threshold': -3.5, 'micro_mult': 1000.0, 'hedge_qty': 1, 'hedge_by_value': False},
    '日圓(JPY)': {'symbols': ['MJY', 'JP', 'JPU'], 'hedge_sym': 'MJY', 'upper_threshold': 3.5, 'lower_threshold': 1.0, 'micro_mult': 1250000.0, 'hedge_qty': 1, 'hedge_by_value': False},
    '歐元(EUR)': {'symbols': ['M6E', 'EU'], 'hedge_sym': 'M6E', 'upper_threshold': 3.0, 'lower_threshold': 1.0, 'micro_mult': 12500.0, 'hedge_qty': 1, 'hedge_by_value': False},
    '天然氣(NG)': {'symbols': ['MNG', 'LN'], 'hedge_sym': 'MNG', 'upper_threshold': 3.7, 'lower_threshold': 1.3, 'micro_mult': 1000.0, 'hedge_qty': 1, 'hedge_by_value': False},
    '銅(HG)': {'symbols': ['MHG', 'HXE'], 'hedge_sym': 'MHG', 'upper_threshold': 3.5, 'lower_threshold': 2.0, 'micro_mult': 2500.0, 'hedge_qty': 1, 'hedge_by_value': False},
    '原油(CL)': {'symbols': ['MCL', 'LO'], 'hedge_sym': 'MCL', 'upper_threshold': -2.0, 'lower_threshold': -3.5, 'micro_mult': 100.0, 'hedge_qty': 1, 'hedge_by_value': False},
    '小羅素(RTY)': {'symbols': ['M2K', 'RTO', 'RT'], 'hedge_sym': 'M2K', 'upper_threshold': 3.5, 'lower_threshold': 1.3, 'micro_mult': 5.0, 'hedge_qty': 1, 'hedge_by_value': False},
    '小標(ES)': {'symbols': ['MES', 'EWN', 'EW1', 'EWQ', 'ESU'], 'hedge_sym': 'MES', 'upper_threshold': 3.5, 'lower_threshold': 1.3, 'micro_mult': 5.0, 'hedge_qty': 1, 'hedge_by_value': False},
    '小納(NQ)': {'symbols': ['MNQ', 'QN', 'NQ'], 'hedge_sym': 'MNQ', 'upper_threshold': 3.5, 'lower_threshold': 1.3, 'micro_mult': 2.0, 'hedge_qty': 1, 'hedge_by_value': False},
    '黃金(Gold)': {'symbols': ['1OZ', 'GDX', 'GDXU'], 'hedge_sym': '1OZ', 'upper_threshold': 2.0, 'lower_threshold': -0.8, 'micro_mult': 1.0, 'hedge_qty': 1, 'hedge_by_value': True},
    '台指(TMF)': {'symbols': ['MTX', 'TMF', 'MXF', 'TX'], 'hedge_sym': 'TMF', 'upper_threshold': 3.0, 'lower_threshold': 1.0, 'micro_mult': 1.0, 'hedge_qty': 1, 'hedge_by_value': False}
}
_DEFAULT_SEND_WEBHOOK = False
...
def load_hedge_config() -> dict:
    """從 .env 的 HEDGE_CONFIG_JSON 讀取設定；若不存在則用預設值，並立刻寫回 .env。"""
    raw = os.getenv("HEDGE_CONFIG_JSON", "")
    if not raw:
        print("⚠️ .env 未設定 HEDGE_CONFIG_JSON，使用內建預設值並寫入 .env")
        persist_env_var("HEDGE_CONFIG_JSON", serialize_hedge_config(_DEFAULT_HEDGE_CONFIG))
        parsed = _DEFAULT_HEDGE_CONFIG
    else:
        try:
...
if os.getenv("SEND_WEBHOOK") is None:
    persist_env_var("SEND_WEBHOOK", "true" if _DEFAULT_SEND_WEBHOOK else "false")
SEND_WEBHOOK = env_bool("SEND_WEBHOOK", _DEFAULT_SEND_WEBHOOK)
