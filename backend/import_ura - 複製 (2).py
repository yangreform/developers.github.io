...
def init_ura_tables():
    ...
    # 🌟 建立私宅租金表 (租金本身就有 noOfBedrooms，不用新增)
    cursor.execute('''
    ...
    for batch in range(1, 5):
        ...
        for project in data:
            ...
            for t in project.get('transaction', []):
                ...
                sale_records.append((p_name, s_name, c_date, calculated_type, f_range, price))
                
        # 批次寫入 SQLite 以提升速度
        cursor.executemany(
            "INSERT INTO ura_transactions (project, street, contractDate, flat_type, floorRange, price) VALUES (?, ?, ?, ?, ?, ?)", 
            sale_records
        )
        ...
        try:
            ...
            for project in data:
                ...
                for r in project.get('rental', []):
                    ...
                    # 🌟 租金資料直接提供房間數
                    bedrooms = r.get('noOfBedrooms', '未知房間數')
                    if bedrooms == 'NIL': bedrooms = '未知房間數'
                    
                    # 🌟 如果有房間數，就寫成 4-Bedroom 格式
                    if '未知' not in bedrooms:
                        flat_type = f"{bedrooms}-Bedroom"
                    else:
                        flat_type = bedrooms

                    rent_records.append((p_name, s_name, l_date, flat_type, rent))
                    
            cursor.executemany(
                "INSERT INTO ura_rentals (project, street, leaseDate, flat_type, rent) VALUES (?, ?, ?, ?, ?)", 
                rent_records
            )
